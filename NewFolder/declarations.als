...
sig NetworkEndpoint{ 
  neOwner: Principal // someone in control of (the processes running at) the network endpoint
}
...
sig HTTPServer extends HTTPConformist{
  owner:Principal // owner of the HTTPClient process
}
...
lone sig c200,c401, c403 extends Status{}
...
sig HTTPTransaction {
	...
	cause : lone HTTPTransaction + RequestAPI, // causal chain on the same browser through HTTP redirects
	protocolPrecursor : lone HTTPTransaction + RequestAPI // causal chain that traces the steps stipulated by protocols
}{
	some resp implies {
...
sig WWWAuthnHeader extends HTTPResponseHeader{}{
  all resp:HTTPResponse| (some (WWWAuthnHeader & resp.headers)) => resp.statusCode in c401+c403
}
