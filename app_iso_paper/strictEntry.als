...
//fixed version
sig StrictEntryBrowserFixed extends Firefox3{}

fact StrictEntryEnforcementFixed{
       all sc:ScriptContext | sc.location=StrictEntryBrowserFixed implies 
           no requ:sc.transactions.req | { 
                  requ.path=SENSITIVE                        //if the request is to a non-entry-point path
                  isRequestToStrictEntryOrigin[requ]     //in a strict entry origin
                  not (lone involvedOrigins[req.requ])  //then there must not be any external origins involved
                                                                        //in the request (e.g. no redirects)
           }
}
...
} for 8 but 0 ACTIVEATTACKER, 1 WEBATTACKER, exactly 1 StrictEntryBrowser, exactly 1 StrictEntryOrigin, 1 SENSITIVE, 0 StrictEntryBrowserFixed // 0 ORIGINAWARE, 0 GOOD, 0 SECURE ,0 Secret, //1 HTTPClient//, 2 Origin, 0 PreFlightRequest, 0 CORSRequestHeader, 0 CORSResponseHeader, 0 Secret//,8 Time//, 1 RequestAPI, 0 Secret
...
//Executing "Check strictEntryProtectsFromSensitiveAccess for 8 but 0 ACTIVEATTACKER, 1 WEBATTACKER, exactly 1 StrictEntryBrowser, exactly 1 StrictEntryOrigin, 1 SENSITIVE, 0 StrictEntryBrowserFixed"
//   Solver=minisat(jni) Bitwidth=4 MaxSeq=7 SkolemDepth=1 Symmetry=20
//   281709 vars. 3855 primary vars. 619351 clauses. 248331ms.
//   Counterexample found. Assertion is invalid. 9285ms.


check strictEntryProtectsFromSensitiveAccessFixed{

	no t:HTTPTransaction | {
		// Transaction is in a StrictEntry BrowserFixed
		some( (transactions.t).location & (StrictEntryBrowserFixed))
		//Intended for an StrictEntry Origin
        some( t.req.host & StrictEntryOrigin)
		// Its a non trivial request
		t.req.method in Method - safeMethods
...
} for 8 but 0 ACTIVEATTACKER, 1 WEBATTACKER, exactly 1 StrictEntryBrowserFixed, exactly 1 StrictEntryOrigin, 1 SENSITIVE, 0 StrictEntryBrowser // 0 ORIGINAWARE, 0 GOOD, 0 SECURE ,0 Secret, //1 HTTPClient//, 2 Origin, 0 PreFlightRequest, 0 CORSRequestHeader, 0 CORSResponseHeader, 0 Secret//,8 Time//, 1 RequestAPI, 0 Secret
//Executing "Check strictEntryProtectsFromSensitiveAccessFixed for 8 but 0 ACTIVEATTACKER, 1 WEBATTACKER, exactly 1 StrictEntryBrowserFixed, exactly 1 StrictEntryOrigin, 1 SENSITIVE, 0 StrictEntryBrowser"
//   Solver=minisat(jni) Bitwidth=4 MaxSeq=7 SkolemDepth=1 Symmetry=20
//   281709 vars. 3855 primary vars. 619351 clauses. 254509ms.
//   No counterexample found. Assertion may be valid. 16370ms.
